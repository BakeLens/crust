...
// TestDetectNoTargets verifies Detect() returns nil/empty when registry has no targets.
func TestDetectNoTargets(t *testing.T) {
	r := &registry.Registry{}
	agents := r.Detect()
	if len(agents) != 0 {
		t.Errorf("Detect() on empty registry returned %d agents, want 0", len(agents))
	}
}

// simpleTarget implements only Target (not ProcessDetectable).
type simpleTarget struct{ name string }

func (s *simpleTarget) Name() string                         { return s.name }
func (s *simpleTarget) Patch(_ int, _ string) error          { return nil }
func (s *simpleTarget) Restore() error                       { return nil }

// TestDetectTargetWithoutProcessDetectable verifies Detect() skips targets that
// do not implement ProcessDetectable.
func TestDetectTargetWithoutProcessDetectable(t *testing.T) {
	r := &registry.Registry{}
	r.Register(&simpleTarget{name: "PlainTarget"})
	agents := r.Detect()
	for _, a := range agents {
		if a.Name == "PlainTarget" {
			t.Error("Detect() should skip targets without ProcessDetectable")
		}
	}
}

// TestDetectEmptyExeNames verifies Detect() skips a FuncTarget with empty ExeNames.
func TestDetectEmptyExeNames(t *testing.T) {
	r := &registry.Registry{}
	r.Register(&registry.FuncTarget{
		AgentName:   "EmptyExe",
		ExeNames:    []string{},
	...
	for _, a := range agents {
		if a.Name == "EmptyExe" {
			t.Error("Detect() should skip targets with empty ExeNames")
		}
	}
}

// testBinName returns the base name of the current test binary without extension.
...
// TestDetectProtectedStatus verifies that a patched target matching a running
// process has status "protected".
func TestDetectProtectedStatus(t *testing.T) {
	binName := testBinName(t)
	r := &registry.Registry{}
	r.Register(&registry.FuncTarget{
		AgentName:   "GoTestAgent",
		ExeNames:    []string{binName},
		PatchFunc:   func(_ int, _ string) error { return nil },
		RestoreFunc: func() error { return nil },
	})
	r.PatchAll(9090, "")
	...
	found := false
	for _, a := range agents {
		if a.Name == "GoTestAgent" {
			found = true
			if a.Status != "protected" {
				t.Errorf("status = %q, want %q", a.Status, "protected")
			}
		}
	}
	if !found {
		t.Error("Detect() did not return GoTestAgent")
	}
}

// TestDetectRunningStatus verifies that an unpatched target matching a running
// process has status "running".
func TestDetectRunningStatus(t *testing.T) {
	binName := testBinName(t)
	r := &registry.Registry{}
	r.Register(&registry.FuncTarget{
		AgentName:   "GoTestAgent",
	...
	for _, a := range agents {
		if a.Name == "GoTestAgent" {
			found = true
			if a.Status != "running" {
				t.Errorf("status = %q, want %q", a.Status, "running")
			}
		}
	}
	if !found {
...
// TestDetectConfiguredStatus verifies that a patched target with no matching
// process has status "configured".
func TestDetectConfiguredStatus(t *testing.T) {
	r := &registry.Registry{}
	r.Register(&registry.FuncTarget{
		AgentName:   "GhostAgent",
		ExeNames:    []string{"nonexistent_agent_xyz"},
		PatchFunc:   func(_ int, _ string) error { return nil },
		RestoreFunc: func() error { return nil },
	})
	r.PatchAll(9090, "")
	...
	found := false
	for _, a := range agents {
		if a.Name == "GhostAgent" {
			found = true
			if a.Status != "configured" {
				t.Errorf("status = %q, want %q", a.Status, "configured")
			}
		}
	}
	if !found {
		t.Error("Detect() did not return GhostAgent")
	}
}

// TestDetectAgentFields verifies DetectedAgent has correct Name, ProcessName,
// and non-empty PIDs when a process is found.
func TestDetectAgentFields(t *testing.T) {
	binName := testBinName(t)
	r := &registry.Registry{}
	r.Register(&registry.FuncTarget{
		AgentName:   "FieldCheck",
		ExeNames:    []string{binName},
		PatchFunc:   func(_ int, _ string) error { return nil },
		RestoreFunc: func() error { return nil },
	})
	...
	found := false
	for _, a := range agents {
		if a.Name == "FieldCheck" {
			found = true
			if a.Name != "FieldCheck" {
				t.Errorf("Name = %q, want %q", a.Name, "FieldCheck")
			}
			if a.ProcessName == "" {
				t.Error("ProcessName should not be empty when process is found")
			}
			if len(a.PIDs) == 0 {
				t.Error("PIDs should not be empty when process is found")
			}
			for _, pid := range a.PIDs {
				if pid <= 0 {
					t.Errorf("PID %d should be > 0", pid)
				}
			}
		}
	}
	if !found {
		t.Error("Detect() did not return FieldCheck agent")
	}
}
